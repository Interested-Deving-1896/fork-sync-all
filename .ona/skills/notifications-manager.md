...
- `.github/workflows/notify-manager.yml` — post-resolver triage + manual dispatch
...
Available actions: `auto-triage`, `list`, `mark-all-read`. Manual dispatch
defaults to read-only `list` with `dry_run=true`; global mark-all operations
also require `confirm_all=true`.

---

## Known-safe auto-triage patterns

These narrowly matched notification titles are eligible for automatic triage
only when their repository belongs to one of the three managed owners:
...
| `rate limit exceeded` / `API rate limit` | Explicit quota exhaustion artifacts |
| `quota exhausted` | Same as above |
...
**Trigger:** after a successful `Resolve CI Failures` run, or manual dispatch
...
- `dry_run`: report without marking read (manual default: `true`)
- `confirm_all`: required for a live `mark-all-read` operation
...
| `clear-notifications.yml` | Manual, dry-run-first repository/all-inbox clear |
| `notify-manager.yml` | **This tool** — post-resolver triage + management interface |
...
poller). Each triggered run costs:
...
- Do NOT add generic auto-triage patterns such as `CI`, `Build`, `Checks`, or
  organization names. Verify every new title is genuinely non-actionable and
  already handled by the control plane.
